...
int main(int argc, const char *argv[]) {
  try {
    ...
    if (settings.rechunk_level >= static_cast<uint32_t>(std::numeric_limits<int>::max())) {
      throw std::out_of_range("Rechunk level is outside the supported exponent range");
    }

    const std::string level = std::to_string(settings.rechunk_level);
    const std::filesystem::path tile_dir =
        std::filesystem::path("data") / ("swissalti3d-" + level + "-level-0");
    const double tile_width = std::ldexp(kSwissCellSize, static_cast<int>(settings.rechunk_level));
    ...
    std::printf(
        "Tracing terrain in %s from LV95 (%.3f, %.3f, %.1f).\n",
        tile_dir.c_str(),
        settings.easting,
        settings.northing,
        settings.elevation
